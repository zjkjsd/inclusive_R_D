...
def fake_d_weight_for_period(period: str) -> float:
    """Return the externally determined fake-D normalization for a period."""
    return 0.87 if period == "run1" else 1.0


def effective_weights_by_run(
    weights: Mapping[str, float], run_periods: tuple[str, ...]
) -> Dict[str, Dict[str, float]]:
    """Expand a fitted weight map into reproducible per-period weights."""
    expanded = {}
    combined = len(run_periods) > 1
    for period in run_periods:
        period_weights = {key: float(value) for key, value in weights.items()}
        if combined and "bkg_fakeD" in period_weights:
            period_weights["bkg_fakeD"] *= fake_d_weight_for_period(period)
...
def main() -> None:
    ...
    # events.
    FIXED_WEIGHTS["bkg_fakeD"] = (
        fake_d_weight_for_period(run) if run != "run1+run2" else 1.0
    )
    ...
    if run == "run1+run2" and "bkg_fakeD" in samples_base:
        ...
        for period in run_periods:
            fake_d.loc[
                fake_d[RUN_PERIOD_COL] == period, RUN_WEIGHT_COL
            ] = fake_d_weight_for_period(period)
    ...
    result["best_weights_by_run"] = effective_weights_by_run(
        result["best_weights"], run_periods
    )
    ...
    output = {
        ...
        "fixed_weights_by_run": effective_weights_by_run(
            FIXED_WEIGHTS, run_periods
        ),
        "weight_representation": {
            "description": (
                "Use the per-period mappings when applying results. For a "
                "combined fit they include the event-level fake-D factor "
                "used by the likelihood."
            ),
            "best_weights_by_run": result["best_weights_by_run"],
        },
        "target": {
        ...
        "optuna": {
            ...
            "best_weights_by_run": effective_weights_by_run(
                optuna_best_weights, run_periods
            ),
