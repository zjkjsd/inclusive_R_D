...
output_file = f'/home/belle/zhangboy/R_D/Signal_MC_ROEx1/B2{decaymode}_nu/Ntuples/bengal_generator_e_{filesize}.root'
output_hash = f'/home/belle/zhangboy/R_D/Signal_MC_ROEx1/B2{decaymode}_nu/Ntuples/hashmap_bengal_generator_e_{filesize}.root'
...
vx.treeFit('anti-B0:De', conf_level=0.00, updateAllDaughters=False, massConstraint=['D+'], ipConstraint=True, path=main_path)
...
ma.fillSignalSideParticleList('e-:sig', 'anti-B0:De -> D+:K2pi ^e-:corrected ?addbrems', path=roe_path)
...
# ma.applyCuts('anti-B0:De', '5<roeMbc(my_mask) and 4.3<B0_CMS2_weMbc and \
...
b_vars = vu.create_aliases_for_selected(
    ...
    + vertex_vars + ['vtxDDSig','DecayHash','DecayHashEx','TagVReChi2','TagVReChi2IP',],
...
ma.variablesToNtuple('anti-B0:De', candidate_vars,filename=output_file, treename='B0', path=main_path)
...
if decaymode == 'D_tau':
    ...
    ma.variablesToNtuple('nu_e:tau', cms_kinematics + vc.kinematics + vc.inv_mass,
                     filename=output_file, treename='nu_e_tau', path=main_path)
    
elif decaymode == 'D_l':    
    ma.variablesToNtuple('e-:B', cms_kinematics + vc.kinematics + vc.inv_mass,
                     filename=output_file, treename='e', path=main_path)
    
    ma.variablesToNtuple('nu_e:B', cms_kinematics + vc.kinematics + vc.inv_mass,
