...
if __name__ == "__main__":
    ...
    # vm.addAlias('nPi0', 'ifNANgiveX(nParticlesInList(pi0:slow), 0.0)') # this will raise an error [ERROR] Exception occurred in exp/run/evt: 1003 / 0 / 21119022  { module: VariablesToNtuple_anti-B0:Dl } [ERROR] Uncaught exception encountered: std::get: wrong index for variant  { module: VariablesToNtuple_anti-B0:Dl }
    ...
    # --------------------------------
    # Photon conversion and J/psi veto
    # --------------------------------
#     ma.fillParticleList('pi+:all_tracks', cut='thetaInCDCAcceptance', path=main_path)
#     ma.reconstructDecay('gamma:veto -> pi+:all_tracks e-:corrected', cut='', path=main_path)
#     ma.copyList('gamma:veto2', 'gamma:veto', path=main_path)
    
#     # Photon V0s
#     ma.fillConvertedPhotonsList('gamma:v0 -> e+ e-', 
#                                 'daughter(0,thetaInCDCAcceptance) and daughter(1,thetaInCDCAcceptance)',
#                                 path=main_path)
#     ma.cutAndCopyList('gamma:v0_good', 'gamma:v0', 
#                       cut='daughter(0,isInList(e-:corrected)) or daughter(1,isInList(e-:corrected))',
#                       path=main_path)
#     ma.rankByLowest('gamma:v0_good', 'M', numBest=1, path=main_path)
#     var_name = 'photonV0_M'
#     ma.variablesToEventExtraInfo('gamma:v0_good',variables={'M':var_name}, path=main_path)
#     vm.addAlias(var_name, f'ifNANgiveX(eventExtraInfo({var_name}), -1.0)')
#     veto_vars.append(var_name)
#     vm.addAlias('isFromGammaV0', f'ifNANgiveX(isDescendantOfList(gamma:v0), -1.0)')
    
#     # Photon veto
#     var_name = 'photonVeto_M'
#     ma.rankByLowest('gamma:veto', 'M', numBest=1, path=main_path)
#     ma.variablesToDaughterExtraInfo('gamma:veto', 'gamma:veto -> pi+:all_tracks ^e-:corrected', 
#                                     variables={'M':var_name}, path=main_path)
#     vm.addAlias(var_name, f'ifNANgiveX(extraInfo({var_name}), -1.0)')
    
#     # J/psi
#     var_name = 'JpsiVeto_M'
#     vm.addAlias('Jpsi_dis_M','abs( M - 3.097 )')
#     ma.rankByLowest('gamma:veto2', 'Jpsi_dis_M', numBest=1, path=main_path)
#     ma.variablesToDaughterExtraInfo('gamma:veto2', 'gamma:veto2 -> pi+:all_tracks ^e-:corrected', 
#                                     variables={'M':var_name}, path=main_path)
#     vm.addAlias(var_name, f'ifNANgiveX(extraInfo({var_name}), -1.0)')
    ...
    ma.applyCuts('anti-B0:Dl', 'vtxReChi2<14', path=main_path)
    ...
    ma.applyCuts('anti-B0:Dl', '4<roeMbc(my_mask) and -5<roeDeltae(my_mask)<5 and \
                  abs(roeCharge(my_mask))<3 and 0<TagVReChi2IP<100', path=main_path) # and 0.2967<weMissPTheta_2<2.7925 
    ...
    b_vars = vu.create_aliases_for_selected(
        ...
        + CSVariables + weVars + vertex_vars + TVVariables + B_mcDaughters_vars
    ...
    l_vars = vu.create_aliases_for_selected(
        list_of_variables= cms_kinematics + vc.kinematics + general_mc_vars + pid_vars + Ancestor_info
        + ['isBremsCorrected','pValue','isCloneTrack','d0_mcPDG','d1_mcPDG','mcSecPhysProc'], #'dr','dz','photonVeto_M','JpsiVeto_M','isFromGammaV0',
        decay_string='anti-B0:Dl =norad=> D+:K2pi ^e-:corrected',
        prefix=['ell'])
    ...
    candidate_vars = ['Ecms', 'nPi0'] + veto_vars + b_vars + D_vars + l_vars
