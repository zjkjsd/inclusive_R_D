...
filesize = sys.argv[2]
input_file = f'/home/belle/zhangboy/R_D/Signal_MC_ROEx1/B2{decaymode}_nu/MC/MC_mu_{filesize}.root'
output_file = f'/home/belle/zhangboy/R_D/Signal_MC_ROEx1/B2{decaymode}_nu/Ntuples/bengal_generator_mu_{filesize}.root'
output_hash = f'/home/belle/zhangboy/R_D/Signal_MC_ROEx1/B2{decaymode}_nu/Ntuples/hashmap_bengal_generator_mu_{filesize}.root'
...
if decaymode == 'D_tau':
    ...
    ma.fillParticleListFromMC('nu_mu:tau', 'abs(genMotherPDG)==15',skipNonPrimaryDaughters=True, path=main_path)
elif decaymode == 'D_l':    
    ma.fillParticleListFromMC('nu_mu:B', 'abs(genMotherPDG)==511',skipNonPrimaryDaughters=True, path=main_path)
    ma.fillParticleListFromMC('mu-:B', 'abs(genMotherPDG)==511',skipNonPrimaryDaughters=True, path=main_path)
...
ma.applyChargedPidMVA(['mu+:mymu'], path=main_path, trainingMode=1, 
                      chargeIndependent=False, binaryHypoPDGCodes=(0, 0))
#ma.applyChargedPidMVA(['mu+:mymu'], path=main_path, trainingMode=0,
...
# store the flightDistance from IP for the D meson after the 1st treeFit
...
# Reconstruct B
ma.reconstructDecay('anti-B0:Dmu -> D+:K2pi mu-:mymu ?addbrems', cut='', path=main_path)
vx.treeFit('anti-B0:Dmu', conf_level=0.0, updateAllDaughters=False, massConstraint=[], ipConstraint=True, path=main_path)
...
# Calculate DOCA(D,l)
ma.calculateDistance('anti-B0:Dmu', 'anti-B0:Dmu -> ^D+:K2pi ^mu-:mymu', "vertextrack", path=main_path)
...
# MC Truth Matching
ma.matchMCTruth('anti-B0:Dmu', path=main_path)

# generate the decay string
main_path.add_module('ParticleMCDecayString', listName='anti-B0:Dmu', fileName=output_hash)
...
####ma.applyCuts('anti-B0:De', 'vtxReChi2<14 and -3.2<deltaE<0 and CMS_E<5.4', path=main_path)
...
ma.buildRestOfEvent('anti-B0:Dmu', fillWithMostLikely=True,path=main_path)
...
ma.appendROEMasks('anti-B0:Dmu', [roe_mask1], path=main_path)


# creates V0 particle lists and uses V0 candidates to update/optimize the Rest Of Event
ma.updateROEUsingV0Lists('anti-B0:Dmu', mask_names='my_mask', default_cleanup=True, selection_cuts=None,
...
roe_path = b2.Path()
deadEndPath = b2.Path()
ma.signalSideParticleFilter('anti-B0:Dmu', '', roe_path, deadEndPath)
...
ma.fillSignalSideParticleList('mu-:sig', 'anti-B0:Dmu -> D+:K2pi ^mu-:mymu ?addbrems', path=roe_path)
...
roel_DOCA_Chi2.append('roel_vtxReChi2_vtx')
...
vx.TagV("anti-B0:Dmu", fitAlgorithm="Rave", maskName='my_mask', path=main_path)

# Continuum Suppression
ma.buildContinuumSuppression(list_name="anti-B0:Dmu", roe_mask="my_mask", path=main_path)
...
# ma.applyCuts('anti-B0:Dmu', '5<roeMbc(my_mask) and 4.3<B0_CMS2_weMbc and \
...
b_vars = vu.create_aliases_for_selected(
    ...
    + roe_multiplicities + roe_nCharged + CSVariables + we + sigDl_DOCA + roel_DOCA_Chi2
    + vertex_vars + ['vtxDDSig','vtxIPDSig','DecayHash','DecayHashEx','TagVReChi2','TagVReChi2IP',],
    decay_string='^anti-B0:Dmu -> D+:K2pi mu-:mymu',
    prefix=['B0'])
...
D_vars = vu.create_aliases_for_selected(
    ...
    + ['dM','BFM','BFInvM','A1FflightDistanceSig_IP',],
    decay_string='anti-B0:Dmu -> ^D+:K2pi mu-:mymu',
    prefix=['D'])

l_vars = vu.create_aliases_for_selected(
    list_of_variables= cms_kinematics + vc.kinematics + vc.mc_truth# + muon_id_weights
    + ['dM','genMotherPDG','nPXDHits'],
    decay_string='anti-B0:Dmu -> D+:K2pi ^mu-:mymu',
    prefix=['mu'])


candidate_vars = ['Ecms','nElectrons90','nMuons90'] + b_vars + D_vars + l_vars
    
ma.variablesToNtuple('anti-B0:Dmu', candidate_vars,filename=output_file, treename='B0', path=main_path)

if decaymode == 'D_tau':
    ma.variablesToNtuple('tau-:B', cms_kinematics + vc.kinematics + vc.inv_mass,
                     filename=output_file, treename='tau_B', path=main_path)
    
    ma.variablesToNtuple('nu_tau:B', cms_kinematics + vc.kinematics + vc.inv_mass,
                     filename=output_file, treename='nu_tau_B', path=main_path)
    
    ma.variablesToNtuple('nu_tau:tau', cms_kinematics + vc.kinematics + vc.inv_mass,
                     filename=output_file, treename='nu_tau_tau', path=main_path)
    
    ma.variablesToNtuple('nu_mu:tau', cms_kinematics + vc.kinematics + vc.inv_mass,
                     filename=output_file, treename='nu_mu_tau', path=main_path)
    
elif decaymode == 'D_l':    
    ma.variablesToNtuple('mu-:B', cms_kinematics + vc.kinematics + vc.inv_mass,
                     filename=output_file, treename='mu', path=main_path)
    
    ma.variablesToNtuple('nu_mu:B', cms_kinematics + vc.kinematics + vc.inv_mass,
                     filename=output_file, treename='nu', path=main_path)
