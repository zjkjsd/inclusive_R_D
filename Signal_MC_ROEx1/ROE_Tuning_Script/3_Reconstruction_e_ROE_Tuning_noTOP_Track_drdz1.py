...
pvalue_tuned = 0.0005
dr1_tuning = [16, 25, 36, 64, 100]
dr1_tuned = 36
dz1_tuning = [4, 9, 16, 25, 36, 49, 64, 100, 225, 400, 625]
dz1_tuned = 16
dr2_tuning = [25, 36, 49, 64, 100]
dr2_tuned = 49
dz2_tuning = [36, 49, 64, 81, 100, 225, 400]
dz2_tuned = 64
dr3_tuning = [25, 36, 49, 64, 81]
dr3_tuned = 49
dz3_tuning = [4, 9, 16, 25, 36, 49, 64, 100, 144]
dz3_tuned = 16
dr4_tuning = [4, 9, 16, 25, 36, 49]
dr4_tuned = 25
dz4_tuning = [9, 16, 25, 36, 49, 64]
dz4_tuned = 36
dr5_tuning = [0.36, 0.64, 1, 2.25, 4]
dr5_tuned = 1
dz5_tuning = [0.64, 1, 2.25]
dz5_tuned = 1
...
if dr1_tuned >0:
    ...
    for dz1 in dz1_tuning:
        ...
        roe_mask1 = ('my_mask',  f'nCDCHits>={ncdchits_tuned} and thetaInCDCAcceptance and pt>{p_t_tuned} and pValue>={pvalue_tuned} and E<5.5 and \
                    [pt<0.15 and formula(dr**2/{dr1_tuned}+dz**2/{dz1_tuned})<1] or \
                    [0.15<pt<0.25 and formula(dr**2/{dr2_tuned}+dz**2/{dz2_tuned})<1] or \
                    [0.25<pt<0.5 and formula(dr**2/{dr3_tuned}+dz**2/{dz3_tuned})<1] or \
                    [0.5<pt<1 and formula(dr**2/{dr4_tuned}+dz**2/{dz4_tuned})<1] or \
                    [pt>1 and formula(dr**2/{dr5_tuned}+dz**2/{dz5_tuned})<1]',
                    ...
                    [clusterE<0.1 and clusterReg==2 and beamBackgroundSuppression>0.2 and hadronicSplitOffSuppression>0 and minC2TDist>25] or \
                    [0.1<clusterE<0.2 and clusterReg==1 and beamBackgroundSuppression>0.4 and hadronicSplitOffSuppression>0 and minC2TDist>25] or \
                    [0.1<clusterE<0.2 and clusterReg==2 and beamBackgroundSuppression>0.4 and hadronicSplitOffSuppression>0 and minC2TDist>25] or \
                    [0.1<clusterE<0.2 and clusterReg==3 and beamBackgroundSuppression>0.4 and hadronicSplitOffSuppression>0 and minC2TDist>25] or \
                    [0.2<clusterE<0.5 and clusterReg==1 and beamBackgroundSuppression>0.4 and hadronicSplitOffSuppression>0 and minC2TDist>20] or \
                    [0.2<clusterE<0.5 and clusterReg==2 and beamBackgroundSuppression>0.4 and hadronicSplitOffSuppression>0 and minC2TDist>20] or \
                    [0.2<clusterE<0.5 and clusterReg==3 and beamBackgroundSuppression>0.4 and hadronicSplitOffSuppression>0 and minC2TDist>20] or \
                    [clusterE>0.5 and clusterReg==1 and beamBackgroundSuppression>0.5 and hadronicSplitOffSuppression>0 and minC2TDist>0] or \
                    [clusterE>0.5 and clusterReg==2 and beamBackgroundSuppression>0.5 and hadronicSplitOffSuppression>0 and minC2TDist>0] or \
                    [clusterE>0.5 and clusterReg==3 and beamBackgroundSuppression>0.5 and hadronicSplitOffSuppression>0 and minC2TDist>0]')
...
large_df.to_csv(f'/home/belle/zhangboy/R_D/Signal_MC_ROEx1/B2{decaymode}_nu/Ntuples/MaskTuning_Track_drdz1_3.csv')
