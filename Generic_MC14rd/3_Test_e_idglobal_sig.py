...
ma.applyCuts(f"e-:corrected", "eID>0.5 and pt>0.2 and thetaInCDCAcceptance and nCDCHits>0 and nPXDHits>0", path=main_path)
ma.applyCuts(f"mu-:mymu", "pidChargedBDTScore(13, ALL)>0.9", path=main_path)

# select events only containing 1 lepton
vm.addAlias('nElectrons50','nParticlesInList(e-:corrected)')
...
# Reconstruct B
ma.reconstructDecay('anti-B0:Dl -> D+:K2pi e-:corrected ?addbrems', cut='', path=main_path)
...
b_vars = vu.create_aliases_for_selected(
    list_of_variables= vc.deltae_mbc + vc.mc_truth + roe_Mbc_Deltae + roe_E_Q 
    + roe_nCharged + we + vertex_vars + ['vtxDDSig','DecayHash','DecayHashEx','TagVReChi2','TagVReChi2IP',],
...
candidate_vars = ['Ecms','nElectrons50','nMuons90',] + b_vars + D_vars + l_vars
